...
function xxvalidarNota(campo) {
	
	col = campo.name.substr(4,1);
	porc_col = 'porc'+col;
	valor = 'document.formu1212.'+porc_col+'.value'; 
	porcentaje = eval(valor);
	//alert(porcentaje);
	//alert (col);
	//campo.value = parseFloat(campo.value);
	/*if ((campo.value.length == 1) || (campo.value.length == 3)){
		campo.value = '0'+campo.value
	}*/
	//campo.value = parseFloat(campo.value);
	if (campo.value > 0 && campo.value < 0.01){// Esta condición se usa solo si se aceptan decimales
		//
		alert("Nota no válida.\n\nEl valor de la calificación debe ser entre 0 y "+porcentaje+".\n\nRecuerde que el valor cero (0) es para inasistentes\ny los valores decimales deben separase con punto(.).");
		campo.value = "";
		campo.focus();
	}
	...
	if (eval(campo.value) > porcentaje){
		alert("Nota no válida.\n\nEl valor de la calificación debe ser: entre 0 y "+porcentaje+".\n\nRecuerde que el valor cero (0) es para inasistentes\ny los valores decimales deben separase con punto(.).");
		campo.value = "";
		/*campo.name
		document.formu1212..focus(); */
	}
}
...
numeva=document.formnotas.cont.value;
valoreva=eval("document.getElementById('porc"+numeva+"').value");
		if (valorcampo > 0 && campo.value < 0.01){// Esta condición se usa solo si se aceptan decimales
		alert("Nota no válida.\n\nEl valor de la calificación debe ser entre 0 y "+valoreva+".\n\nRecuerde que el valor cero (0) es para inasistentes\ny los valores decimales deben separase con punto(.).");
		campo.value = ""
		}
	// En esta condición el nueve(9) es estático, en tu caso debes validarlo con el porcentaje
	// de la evaluación que se este cargando en ese momento.
...
function validarloquefalta(cantidad) {
	...
	//alert('operacion: '+(100-(((cantee-evaintro)*30)+(100-restante))));

	//alert('restante: '+restante+' eval: '+cantee+' evaintro: '+evaintro+' minimo:'+ minimo);
...
function ValidaGral(formu,x,y){
	//alert('form: '+formu.name+' x: '+x+' y: '+y);
	error = 0;
	with (formu){
		for (j=0;j<y;j++){
			for (i=1;i<=x;i++){		
				...
				//nota = eval(campo+'.value');
				nota = document.getElementById(campo).value;
				//alert(nota);
				if ((nota == "") && (tipo == "text")){
					error++;		
					lcolor='#FFFF66';
					document.getElementById(campo).style.background = lcolor;
		...
		if (error > 0){
			alert('Complete los campos marcados con fondo amarillo.');
		}else{
			if (formu.razomoca.value == ''){
				alert('Indique la razón por la cuál realiza esta modificación.');
				error++;
			}
			if (error == 0){
				envia = confirm('¿Está seguro que desea modificar las calificaciones?\n\nVerifique los cambios realizados antes de continuar.');
				if (envia){
					formu.submit();
				}else{
					//alert('Verifique');
...
function m_over(item){
	if (item.className!='moused'){
		moused_style=item.className;
		item.className='moused';
	}
}
function m_out(item){
	if (item.className=='moused'){
		item.className=moused_style
	}
}	