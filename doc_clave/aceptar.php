   ...
   // include_once('../clave/inc/vImage.php');
	...
	//$sede1 = "DRIVER={Centura SQLBase 3.5 32-bit Driver -NT & Win95};SRVR=Server1;DB=usersdb;uid=scael;pwd=c0n_4c4";
		...
		function elUsuarioExiste($sinError) {
			...
			$dSQL = "SELECT userid FROM USUARIOS WHERE userid='$exp_e' ";
			$user->ExecSQL($dSQL);
			if ($user->filas == 1){
				$cv = ($exp_e == $user->result[0][0]); 
		...
		function elUsuarioNoTieneClave(&$sinError) {
			...
			$user = new ODBC_Conn("userdoc","scael","c0n_4c4");
			$dSQL = "SELECT userid FROM USUARIOS WHERE usid='$exp_e' and password is null ";
			$user->ExecSQL($dSQL);
			if ($user->filas == 1){
				$sinError=true;
			}
		...
		function restablecerClave(&$sinError) {
			...
			$Clave = new ODBC_Conn("USERDOC","ASCAEL","adc0n4c4");
						
			$dSQL = "UPDATE usuarios SET password ='$contra' WHERE userid='$exp_e'";
		  ...
		  function agregarUsuario(&$sinError) {
			...
			//$elNombre = $datos_p[3];
			$Clave = new ODBC_Conn("USERDOC","ASCAEL","adc0n4c4");
			$dSQL  = "INSERT INTO usuarios (userid, exp_e, password, nombre, ";
			$dSQL .= "checkmail, hidemenu, hidepass, hidemail, hideqa, ";
			$dSQL .= "hideusers, hidelast, lasttime, times, campus, ";
			$dSQL .= "tipo_usuario) values ('".$exp_e."', '".$exp_e."', '".$contra;
		  ...
		  function imprimirTodoOK() {
			  global $datos_p, $exp_e, $nombres, $mensaje_error;
			?>
              <font size="3"  face="Arial" >
                <p><center><strong> Gracias   <?php echo ' '.$nombres.', '; ?> la Clave ha sido registrada
				</strong>
...
<?php
	 if (!empty($_POST['exp_e']) && (!empty($_POST['contra'])))  {
		$exp_e = $_POST['exp_e'];
		$contra = $_POST['contra'];
		...
		//echo $exp;
		$sinError = true;
		$mensaje_error = 'No se pudo registrar la clave. Por favor, tome nota de la hora actual, los datos del usuario y contacte al personal de soporte en DACE.';
