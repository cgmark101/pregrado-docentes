...
<script language="Javascript" src="md5.js">
  ...
  function validar(f) {
	if ((f.cedula_v.value == "")||(f.contra_v.value == "")) {
		...
		f.vImageCodC.value = "";
		f.lapso.value = f.lapsoP.value;
		f.lapsoP.selectedIndex = 0;
...
<table style="border-collapse:collapse;padding-left:350px;" border="0" cellpadding="0" cellspacing="1" width="85%" ><tr><td>
			<br><table style="padding-left:280px;"class="normal">
				...
				<tr>
					...
					<td>
						<select name="lapsoP" tabindex="3">
							<option value="">SELECCIONE</option>
						
<?php
	$conex = new ODBC_Conn("centura-dace","c","c");
	/*$sql = "SELECT DISTINCT lapso, @SUBSTRING(lapso,0,4) ";
	$sql.= "FROM dace004 ";
	$sql.= "WHERE status IN ('0','1') AND @SUBSTRING(lapso,0,4) >= 2014 ";
	$sql.= "AND @SUBSTRING(lapso,5) IN ('1','2','1I','U') ";
	$sql.= "ORDER BY 1 DESC";*/
	
	$sql = "SELECT DISTINCT his_lap, @SUBSTRING(his_lap,0,4) ";
	$sql.= "FROM his_act ";
	$sql.= "WHERE @SUBSTRING(his_lap,0,4) >= ".(date('Y')-3)." ";
	$sql.= "AND @SUBSTRING(his_lap,5) IN ('1','2','1I','U','E') ";
	$sql.= "AND his_lap NOT IN ('2014-1','2014-2') ";
	$sql.= "ORDER BY 1 DESC";
	...
	for ($i=0; $i < $conex->filas; $i++) {
		...
		$lapsoP = $conex->result[$i][0];
		
		$tLapso	= $lapsoP;
		echo "<option value=\"$lapsoP\">".$tLapso."</option>\n";
	}				
?>
						</select>				
					</td>
...
<br>
			<table style="padding-left:280px;" class="normal" width="100%" border="0">
				<tr>
					<td width="100%">
						C&oacute;digo de Seguridad:&nbsp;&nbsp;<img src="inc/img.php?size=4" height="30px" style="vertical-align: middle;">&nbsp;&nbsp;<input name="vImageCodC" size="5" tabindex="4" type="text"> 
					</td>
				<tr>
				<tr>
					<td>
						<input value="Entrar" name="b_enviar" tabindex="5" type="submit">
					</td>
				</tr>
			</table>
<br>
			  ...
			  <input value="x" name="lapso" type="hidden">
