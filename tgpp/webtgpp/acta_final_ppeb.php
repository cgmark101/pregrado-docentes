...
$pdf->ezText('HOY,_______________________, REUNIDOS EN________________________________, PRESIDIDO POR EL(LA) PROFESOR(A): '.$prof_aux[0]["NOMBRE"].' '.$prof_aux[0]["APELLIDO"].
             ', SE DIO INICIO AL ACTO DE PRESENTACIÓN ORAL PÚBLICA DE'.$ART[0]["ART"].' '.$asi[0]["TIPO"].' TITULAD'.$ART[0]["LETRA"].': '.$tablaaux[0]["n_proyecto"].'.',10,array('justification' => 'full','spacing' =>1.5));
$pdf->ezSetDy(-5);
...
if (empty($tablaaux[0]["cedula_TI"])){ // Si no tiene tutor industrial
	$data = array(
...
if (empty($tablaaux[0]["cedula_TI"])){// Si no tiene tutor industrial
	...
	array('MIEMBROS'=>'TUTOR ACADÉMICO', 'NYA'=>'','CI'=>'','FIRMA'=>''),
