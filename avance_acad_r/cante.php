...
for($a=0;$a < count($agregados);$a++) {
	...
	if (count($conex->result) == 0){// Si no esta en el acta lo inserto.
		$mSQL = "INSERT INTO N_ESTU (acta, lapso, c_asigna, exp_e, total100, total9, ccca, ucca) VALUES ('".$acta."', '".$lapso."', '".$c_asigna."', '".$agregados[$a][0]."', '0', '0', '".$ulti."', '".$ulti."')";
		$conex->ExecSQL($mSQL,__LINE__,true);
	}	
}
      ...
      <form name="form1" method="post" onsubmit="return validaropcion();" >
        <label>
          <select name="select" id="selectop" class="select.peq">
			...
			<!-- <option style="color:#FF0000;font-weight:bold" disabled>Disponible del 27/06/2014 al 18/07/2014</option> -->
			<option value="999" disabled>Confirmar el Cierre del Acta</option>
			</optgroup>
          </select>
        </label>
        <p>
