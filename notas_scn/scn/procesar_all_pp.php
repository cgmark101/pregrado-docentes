...
//consulto estudiantes en pp_temp
$conex = new ODBC_Conn($ODBC_name,$usuario_db,$password_db,TRUE,$log);
$mSQL  = "SELECT acta, lapso, c_asigna, exp_e, calificacion, status, seccion, acta2, observacion FROM sc_pp_temp";     
...
//busco la cantidad de inscritos en pp_temp
	$conex = new ODBC_Conn($ODBC_name,$usuario_db,$password_db,TRUE,$log);
...
$conex = new ODBC_Conn($ODBC_name,$usuario_db,$password_db,TRUE,$log);
$mSQL  = "SELECT DISTINCT acta,lapso,seccion,c_asigna FROM sc_pp_temp";     
$conex->ExecSQL($mSQL,__LINE__,false);
$result = $conex->result;
...
## FILTRAR POR STATUS Y NO POR OBSERVACION
//consulto aprobados en tg_temp
$conex = new ODBC_Conn($ODBC_name,$usuario_db,$password_db,TRUE,$log);
$mSQL  = "SELECT observacion FROM sc_pp_temp WHERE status='0' and acta='$his_act[$i]'";     
$conex->ExecSQL($mSQL,__LINE__,false);
$fila = $conex->filas;
$his_apr[$i]=$fila; 
//echo $his_apr[$i];
...
//consulto aprobados en tg_temp
$conex = new ODBC_Conn($ODBC_name,$usuario_db,$password_db,TRUE,$log);
$mSQL  = "SELECT observacion FROM sc_pp_temp WHERE status IN (1,'I') and acta='$his_act[$i]'";     
$conex->ExecSQL($mSQL,__LINE__,false);
$fila = $conex->filas;
$his_apl[$i]=$fila; 
//echo $his_apl[$i];
