...
if (isset($_POST['update'])){
	...
	$mSQL = "delete from dotros_ch where CI_D='$doc'";
	$Chijos->ExecSQL($mSQL);
}
...
for ($i = 1; $i <= $cargos; $i++) {
		...
		$cargo=$_POST['cargo_' . $i];
		$desde=$_POST['desde_' . $i];
		$hasta=$_POST['hasta_' . $i];
		
		//INSERTA LOS DATOS EN LA TABLA
		$Cmat_ah = new ODBC_Conn("FDOCENTE","N","N");
		$mSQL = "INSERT INTO DOTROS_CH (CI_D,CARGO,DESDE,HASTA,NUM)";
		$mSQL= $mSQL." VALUES ('$doc','$cargo','$desde','$hasta','$i')";
