...
if( ($_SERVER['HTTP_REFERER']==$raizDelSitio.'planilla_r.php') && (isset($_POST['cedula'])) ){
	...
	$result = $conex->result;
	if ($conex->filas == 0){
		die ("<script languaje=\"javascript\"> history.back(alert('El número de acta no corresponde con la cédula. Por favor verifique e intente nuevamente.')); </script>");
	}
}
...
#De aquí en adelante entra en funcionamiento tu código
...
<tr><td><?PHP include("datosdemateria.php"); ?></td></tr>
  <tr>
	...
	<?php
	echo  '<table border="1" cellspacing="1" class="datotabla" width="100%" style="border-collapse:collapse;">
...
<table>
  ...
  <tr>
	...
	<tr><td class="datona">
	<p class="enc_p"><strong>MODIFICACIONES A LA DESCRIPCION DE LOS TEMAS</strong></p>
	<hr size="1">
		<?php
		//require_once("inc/odbcss_c.php");
		$cont=0;
...
if (($ultima == $total) && ($total != "") && ($ultima != "")) {
	...
	if($total100<100 || $total100>100) {
		...
		echo "<script language=\"javascript\">alert('ATENCION: Esta es una vista previa de su acta final. Por favor verifique que toda la información contenida en ella sea correcta. Presione Confirmar para CERRAR el acta. Presione Regresar para volver al menú de opciones.');document.enviar.action='cerrar.php';document.enviar.submit();</script>";
	}

	
}else {
	echo "<script language=\"javascript\">alert('Lo siento usted no ha completado su Avance Académico en un 100%.');document.enviar.action='cante.php';document.enviar.submit();</script>";
}

//echo "<script language=\"javascript\">alert('ATENCION: Esta es una vista previa de su acta final. Por favor verifique que toda la información contenida en ella sea correcta. Presione Confirmar para CERRAR el acta. Presione Regresar para volver al menú de opciones.');document.enviar.submit();</script>";
      ...
      <form name="form1" method="post" onsubmit="return validaropcion();" >
        <label>
          <select name="select" id="selectop" class="select.peq">
			...
			<option style="color:#FF0000;font-weight:bold" disabled>Disponible del 22/09/2014 al 29/09/2014</option>
